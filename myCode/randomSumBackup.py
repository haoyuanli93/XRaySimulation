...
# Define the runList
run_to_process = [174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187]
runNum = len(run_to_process)

# Loop through the runs to get all the event time and fiducial
eventTimeAll = []
fiducialAll = []
for runIdx in range(runNum):
    with h5.File("/reg/d/psdm/xpp/xppc00120/results/smalldata_output_hy/xppc00120_Run0{}.h5".format(
            run_to_process[runIdx])) as datafile:
        eventTimeAll.append(np.copy(np.array(datafile['event_time'])))
        fiducialAll.append(np.copy(np.array(datafile['fiducials'])))
...
dsList = [psana.DataSource('exp={}:run={}:idx'.format(expname, run)) for run in run_to_process]
...
for pairIdx in range(pairNum):
    ...
    if pairIdx == 0:
        ...
        et1 = psana.EventTime(int(eventTimeAll[run1][idx1]), fiducialAll[run1][idx1])
        evt1 = runList[run1].event(et1)
    ...
    # Load the second pattern
    et2 = psana.EventTime(int(eventTimeAll[run2][idx2]), fiducialAll[run2][idx2])
    evt2 = runList[run2].event(et2)
    # evt1 = runList[run1].event(timesList[run1][idx1])
    img2 = detList[run2].calib(evt2)
